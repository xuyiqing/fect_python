...
// ---- Minimal helpers extracted for MC ----
...
// Demean Y by additive FE under given force
static py::dict Y_demean_py(py::array_t<double> Y, int force) {
  ...
  double mu_Y = 0.0; for (ssize_t i = 0; i < T * N; ++i) mu_Y += Yp[i]; mu_Y /= double(T * N);
  std::vector<double> alpha_Y((size_t)N, 0.0); std::vector<double> xi_Y((size_t)T, 0.0);
  if (force == 0) {
    for (ssize_t t = 0; t < T; ++t) for (ssize_t n = 0; n < N; ++n) YYp[idx((size_t)t, (size_t)n, (size_t)N)] = Yp[idx((size_t)t, (size_t)n, (size_t)N)] - mu_Y;
  } else if (force == 1) {
    for (ssize_t n = 0; n < N; ++n) { double s = 0.0; for (ssize_t t = 0; t < T; ++t) s += (Yp[idx((size_t)t, (size_t)n, (size_t)N)] - mu_Y); alpha_Y[(size_t)n] = s / double(T); }
    for (ssize_t t = 0; t < T; ++t) for (ssize_t n = 0; n < N; ++n) YYp[idx((size_t)t, (size_t)n, (size_t)N)] = (Yp[idx((size_t)t, (size_t)n, (size_t)N)] - mu_Y) - alpha_Y[(size_t)n];
  } else if (force == 2) {
    for (ssize_t t = 0; t < T; ++t) { double s = 0.0; for (ssize_t n = 0; n < N; ++n) s += (Yp[idx((size_t)t, (size_t)n, (size_t)N)] - mu_Y); xi_Y[(size_t)t] = s / double(N); }
    for (ssize_t t = 0; t < T; ++t) for (ssize_t n = 0; n < N; ++n) YYp[idx((size_t)t, (size_t)n, (size_t)N)] = (Yp[idx((size_t)t, (size_t)n, (size_t)N)] - mu_Y) - xi_Y[(size_t)t];
  } else {
    // approximate two-way: alternating projections on all entries
    ...
    for (ssize_t t = 0; t < T; ++t) for (ssize_t n = 0; n < N; ++n) { double v = Yp[idx((size_t)t,(size_t)n,(size_t)N)] - mu; v -= a[(size_t)n]; v -= x[(size_t)t]; YYp[idx((size_t)t,(size_t)n,(size_t)N)] = v; }
  }
  py::array_t<double> alpha_arr({(py::ssize_t)N, (py::ssize_t)1}); py::array_t<double> xi_arr({(py::ssize_t)T, (py::ssize_t)1});
...
// Fill missing entries in E with FE; return updated FE using soft-impute shrinkage (MC core)
static py::dict ife_mc_py(py::array_t<double> E, int force, int r, double lambda) {
  ...
  if (r > 0) {
    ...
    for (ssize_t k = 0; k < kmax; ++k) {
      double sk = Sp[k] - lambda; if (sk < 0.0) sk = 0.0;
      for (ssize_t t = 0; t < T; ++t) Ush[t * kmax + k] = Up[t * (ssize_t)U.shape(1) + k] * sk;
    }
    for (ssize_t t = 0; t < T; ++t) {
      for (ssize_t n = 0; n < N; ++n) {
        double sacc = 0.0;
        for (ssize_t k = 0; k < kmax; ++k) sacc += Ush[t * kmax + k] * Vhp[k * N + n];
        FEip[idx((size_t)t,(size_t)n,(size_t)N)] = sacc;
      }
    }
    if (kmax > 0) {
      double* Vd = VNT.mutable_data();
      for (ssize_t i = 0; i < kmax; ++i) { double sk = Sp[i] - lambda; if (sk < 0.0) sk = 0.0; Vd[i * kmax + i] = sk; }
    }
  }
...
static py::array_t<double> E_adj_py(py::array_t<double> E, py::array_t<double> FE, py::array_t<double> I) {
  ...
  const double* Ep = E.data(); const double* FEp = FE.data(); const double* Ip = I.data(); double* EEp = EE.mutable_data();
  for (ssize_t t = 0; t < T; ++t) for (ssize_t n = 0; n < N; ++n) { size_t k = (size_t)t * (size_t)N + (size_t)n; EEp[k] = (Ip[k] == 0.0 ? FEp[k] : Ep[k]); }
  return EE;
}
...
static py::array_t<double> initial_fit_py(py::array_t<double> Y, py::array_t<double> I_control, py::array_t<double> X, int force) {
  ...
  const double* Yp = Y.data(); const double* IIp = I_control.data();
  // FE on controls
  ...
  for (ssize_t t=0; t<T; ++t) for (ssize_t n=0; n<N; ++n) { double v = mu; if (force==1||force==3) v += a[(size_t)n]; if (force==2||force==3) v += x[(size_t)t]; FEp[idx((size_t)t,(size_t)n,(size_t)N)] = v; }
...
py::tuple mc_predict_cf_r(py::array_t<double, py::array::c_style | py::array::forcecast> Y,
  ...
  if (!hasX) {
    ...
    while (true) {
      ...
      ife_out = ife_mc_py(YY, force, r, lambda);
    ...
    while (true) {
      ...
      ife_out = ife_mc_py(U, force, r, lambda);
...
PYBIND11_MODULE(_mc, m) {
  m.doc() = "Matrix Completion (MC) predictor extracted from IFE module";
  m.def("mc_predict_cf_r", &mc_predict_cf_r,
        py::arg("Y"), py::arg("D"), py::arg("I"), py::arg("X") = py::none(),
        py::arg("force") = std::string("two-way"), py::arg("r") = 1, py::arg("lambda") = 0.0);
}
