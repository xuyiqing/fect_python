...
def _esplot_like(
    ...
    if mask_valid_ci.any():
        ...
        ax.vlines(xv, cilv, ciuv, color="#000000", linewidth=1.6, zorder=2)
        yerr_low = np.clip(yv - cilv, a_min=0.0, a_max=None)
        yerr_high = np.clip(ciuv - yv, a_min=0.0, a_max=None)
        yerr = np.vstack([yerr_low, yerr_high])
        ax.errorbar(xv, yv, yerr=yerr, fmt='none', ecolor="#000000", elinewidth=1.2, capsize=3, capthick=1, zorder=2)
    ax.scatter(x, y, s=25, color="#000000", zorder=3)
    ...
    ax.set_xlabel("Time Since the Treatment's Onset")
...
def plot_result(
    ...
    if type not in ("gap", "calendar"):
        raise NotImplementedError("Only type='gap' and type='calendar' are supported.")
